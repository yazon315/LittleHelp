...
### Подключение к существующему репозиторию по ссылке.
...
### Перенос локального проекта в репозиторий.
...
### Просмотр измененных файлов в локальном репозитории:
...
В удаленном репозитории новая ветка передается через `git push -u` (при дальнейшей работе с веткой флаг `-u` можно опустить):
```bash
$ git push -u origin имя_ветки
```


### Переключение между ветками репозитория
...
### Устранение конфликтов влияния веток
